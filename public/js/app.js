...
// @see https://github.com/ajlkn?tab=repositories
window.breakpoints = __webpack_require__(30);
window.browser = __webpack_require__(31);
...
var breakpoints = function () {
			...
			if (!(query in _.media)) {
				...
				if (breakpoint && breakpoint in _.list) {
					...
					if (Array.isArray(a)) {
						...
						if (!isNaN(x)) units = a[0].substr(String(x).length);else if (!isNaN(y)) units = a[1].substr(String(y).length);else return;
   ...
   */
		...
		_.init(list);
	};__._ = _;__.on = function (query, handler) {
		_.on(query, handler);
...
(function (root, factory) {
	...
	// Node.
	else if ((typeof exports === 'undefined' ? 'undefined' : _typeof(exports)) === 'object') module.exports = factory();
